...
const tasks = [
  ...
  {
    name: "UI Tests",
    command: `npm run test:ui -- --reporter=json --outputFile=test-results/ui.json --run --pool=threads --poolOptions.threads.maxThreads=${maxWorkers}`,
    emoji: "🎨",
  },
  ...(withIntegration
    ? [
  ...
  try {
    ...
    if (existsSync("./test-results/unit.json")) {
      const unitData = JSON.parse(
        readFileSync("./test-results/unit.json", "utf-8"),
      );
      unitData.testResults?.forEach(
        (file: {
          name: string;
    ...
    if (existsSync("./test-results/ui.json")) {
      const uiData = JSON.parse(
        readFileSync("./test-results/ui.json", "utf-8"),
      );
      uiData.testResults?.forEach(
        (file: {
          name: string;
          assertionResults?: Array<{
            fullName?: string;
    ...
    if (withIntegration && existsSync("./test-results/integration.json")) {
      const integrationData = JSON.parse(
        readFileSync("./test-results/integration.json", "utf-8"),
      );
      integrationData.testResults?.forEach(
        (file: {
          name: string;
          assertionResults?: Array<{
            fullName?: string;
...
async function runAllTasks() {
  ...
  const testDetails = await parseTestResults();
  writeFileSync(
    "./test-results/reports/test-details.json",

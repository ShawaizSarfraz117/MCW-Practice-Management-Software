...
3.  **`ClientGroupMembership.role`:**
    - **Refined List:** Common roles include 'Primary Patient' (the individual whose clinical record is the focus for that group's services), 'Spouse/Partner', 'Child', 'Parent/Guardian', 'Emergency Contact', 'Other Family Member'. The specific set should be configurable or a well-defined list. The "Contacts" tab in the video [01:42] directly supports the need to define these roles.
...
9.  **Client-Specific Default Services/Rates (New from Video Context [00:42]):**
...
3.  **`PracticeSettings` vs. `ClientReminderPreference` & Portal Activation ("Video 3" Insight):**
    - **Interaction Logic (Refined with "Video 2" insights & Video 3 [01:04, 01:59]):**
      ...
      2.  **Portal Activation & Contact Verification:** Clients typically set/manage their `ClientReminderPreference`s via the client portal after they have completed an initial staff-initiated invitation and portal activation process (Video [01:59]), during which they also verify their primary contact methods.
...
**New Questions (from "Video 2" & Video 3 context / analysis):** 18. **Granular SMS Opt-In Storage**: How exactly should granular SMS opt-ins (e.g., appointments vs. billing) be stored? Multiple `ClientReminderPreference` records for the same client+SMS channel but different `reminder_type`s? Or a JSON object within a single `ClientReminderPreference.options` field? 19. **ClientPortalInvitation Model Details**: If we add `ClientPortalInvitation`, what specific fields are needed (e.g., `token` (unique, indexed), `email_sent_to`, `expires_at`, `status` ('SENT', 'ACTIVATED', 'EXPIRED', 'ERROR'), `client_id` (once linked))? 20. **ClientPortalAccount Model Details**: If added, what fields beyond `hashed_password` (e.g., `client_id` (FK, unique), `last_login_at`, `failed_login_attempts`, `account_status` ('PENDING_ACTIVATION', 'ACTIVE', 'SUSPENDED', 'LOCKED'), `password_reset_token`, `password_reset_expires_at`)? 21. **Referral Source Reporting**: What specific reports are envisioned for referral sources (e.g., count of clients per source, conversion rate from referral to active client, revenue generated by referral source)? This will help define the fields for the `ReferralSource` entity. 22. **Conflict Resolution UI for Unavailability**: When a clinician's new unavailability conflicts with existing client appointments, what information should the UI present to staff to help them resolve it (e.g., list of conflicting appointments, client contact info, direct links to reschedule)? 23. **Automated Intake Form Assignment Rules Configuration**: How complex are the rules for assigning intake `SurveyTemplate`(s)? Is it purely `PracticeService.id`, or can it involve client age (minor/adult) or other factors from `AppointmentRequest`? Where are these rules configured (e.g., in `PracticeSettings` as a JSON object, or a dedicated table)? 24. **Insurance Authorization Details (Video [00:21]):** What specific fields are essential for an `InsuranceAuthorization` model (e.g., auth number, service codes covered, date ranges, number of sessions approved/used, status)? How does this link to specific appointments or to a `ClientInsurancePolicy`? 25. **Client-Specific Default Services/Rates (Video [00:42]):** How should these client-specific defaults interact with/override other rate tiers (`PracticeService`, `ClinicianService`, `ClientGroupService`)? Is this for every service or just commonly used ones for that client? What is the UI for managing these? 26. **Stripe Payment Setup Details (Video [00:21]):** The video shows "Set Up Stripe". Beyond storing a payment method token, what other Stripe-related IDs or configurations might need to be stored (e.g., Stripe Customer ID associated with the `Client` or `ClientGroup`)? Is there a per-transaction setup or a one-time client setup? 27. **CMS Fields for Claims (Video [00:21]):** "CMS fields" are mentioned under insurance details. What are these specific fields? Do they belong on an `InsurancePolicy` model, an `Appointment` (if claim-specific), or a dedicated `Claim` model that would aggregate data for submission? 28. **Tracking "Exported" vs. "Sent to Client" for Billing Docs (Video [02:50]):** How should the schema formally differentiate between a document being exported for accounting (e.g., `Invoice.is_exported`) versus being sent/made available to the client (e.g., `Invoice.delivery_method` or `Invoice.delivery_status`)? Are there distinct workflows or statuses?

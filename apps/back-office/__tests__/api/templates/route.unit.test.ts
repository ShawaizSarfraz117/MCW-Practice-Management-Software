...
describe("Templates API", () => {
  ...
  describe("GET /api/templates", () => {
    it("should get all templates", async () => {
      const mockTemplates = [mockTemplate, { ...mockTemplate, id: "test-id-2" }];
    ...
    it("should filter templates by type", async () => {
      ...
      const response = await GET(createRequest("/api/templates?type=Test%20Type"));
  ...
  describe("PUT /api/templates/[id]", () => {
    ...
    it("should return 404 if template does not exist", async () => {
      ...
      const response = await PUT(
        createRequestWithBody("/api/templates/non-existent-id", { name: "Updated Name" }),
        { params: { id: "non-existent-id" } }
      );

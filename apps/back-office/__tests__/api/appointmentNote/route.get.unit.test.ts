...
describe("appointmentNote API - GET Unit Tests", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should fetch notes by appointment_id", async () => {
    ...
    expect(data[0].id).toBe("789e0123-e89b-12d3-a456-426614174000");
  ...
  it("should fetch specific note by appointment_id and template_id", async () => {
    prismaMock.surveyAnswers.findFirst.mockResolvedValue(
      mockSurveyAnswer as unknown as SurveyAnswers,
    );

    const request = createRequest(
      "/api/appointmentNote?appointment_id=abc12345-e89b-12d3-a456-426614174000&template_id=123e4567-e89b-12d3-a456-426614174000",
    );
    const response = await GET(request);
  ...
  it("should return null when note not found for appointment_id and template_id", async () => {
    prismaMock.surveyAnswers.findFirst.mockResolvedValue(null);

    const request = createRequest(
      "/api/appointmentNote?appointment_id=nonexistent&template_id=123e4567-e89b-12d3-a456-426614174000",
    );
    const response = await GET(request);

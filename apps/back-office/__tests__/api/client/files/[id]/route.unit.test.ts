...
import { generateDownloadUrl, deleteFromAzureStorage } from "@/utils/azureStorage";
...
describe("Client Files [id] API Unit Tests", () => {
  ...
  describe("GET /api/client/files/[id]", () => {
    ...
    it("should return download URL for client group file", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(mockFile);
      ...
      (generateDownloadUrl as unknown as Mock).mockResolvedValue("https://storage.azure.com/test-sas-url");
    ...
    it("should return 404 for non-existent file", async () => {
      // Arrange
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(null);
      (prisma.clientFiles.findFirst as unknown as Mock).mockResolvedValue(null);
    ...
    it("should generate PDF for survey answers", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(null);
      (prisma.clientFiles.findFirst as unknown as Mock).mockResolvedValue(mockClientFile);
      (prisma.surveyTemplate.findUnique as unknown as Mock).mockResolvedValue(mockSurveyTemplate);
    ...
    it("should handle download URL generation errors", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(mockFile);
      (generateDownloadUrl as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("Azure error"));
    ...
    it("should handle errors gracefully", async () => {
      // Arrange
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockRejectedValueOnce(
  ...
  describe("DELETE /api/client/files/[id]", () => {
    ...
    it("should delete client group file without shared instances", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(mockFile);
      (prisma.$transaction as unknown as Mock).mockImplementation(async (fn) => {
        await fn(prisma);
      });
    ...
    it("should require confirmation for files with shared instances", async () => {
      ...
      const mockFile = {
        ...
        ClientFiles: [
          ClientFilesFactory.build({ id: "shared1" }),
          ClientFilesFactory.build({ id: "shared2" }),
        ],
      };
      ...
      expect(json).toMatchObject({
        ...
        sharedWithCount: 2,
        message: expect.stringContaining("This file will be permanently removed"),
      });
    });
    ...
    it("should unshare client file", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(null);
      (prisma.clientFiles.findFirst as unknown as Mock).mockResolvedValue(mockClientFile);
      (prisma.clientFiles.delete as unknown as Mock).mockResolvedValue(mockClientFile);
  ...
  describe("POST /api/client/files/[id]", () => {
    ...
    it("should handle transaction errors", async () => {
      ...
      (prisma.clientGroupFile.findFirst as unknown as Mock).mockResolvedValue(mockFile);
      (prisma.$transaction as unknown as Mock).mockRejectedValueOnce(new Error("Transaction failed"));

...
import { ShareDocuments, ShareClient } from "@/(dashboard)/share-documents/ShareDocuments";
import { fetchSingleClientGroup, ClientGroupWithMembership } from "@/(dashboard)/clients/services/client.service";
...
async function getClientEmail(clientId: string): Promise<string | null> {
  try {
    ...
    if (response.ok) {
      ...
      const emailContact = contacts.find((c: { contact_type: string; value: string }) => c.contact_type === 'EMAIL');
      ...
      try {
        ...
        if (response && typeof response === 'object' && 'data' in response) {
          ...
          if (clientGroupData.ClientGroupMembership?.length) {
            ...
            const clientsPromises = clientGroupData.ClientGroupMembership.map(async (membership) => {
